    ...
    try {
      ...
      if (src.match('_spec.js$')) {
        if (config.specType === 'jasmine') {
          src_text =
            "var __jasmine = require('/lib/jasmine');var methods = ['spyOn','it','xit','expect','runs','waits','waitsFor','beforeEach','afterEach','describe','xdescribe','jasmine'];methods.forEach(function(method) {this[method] = __jasmine[method];});" +
            src_text;
          ...
          src_text =
            "var chai = require('/lib/chai'); var expect = chai.expect; var assert = chai.assert;\n" +
            "var sinon = require('/lib/sinon');\n" +
            src_text;
        }
        if (config.isAlloy) {
      ...
      } else if (config.runCoverage && !src.match('spec/')) {
        //Instrumenting the application code with istanbul for code coverage
        var instrumentedCode = require('./coverage').instrumentCode(
          src_text,
          src
        );
...
function finalise(file_list, callback) {
  ...
  bundle.pack(file_list.files, function(written) {
    ...
    if (config.isBundle) {
      ...
      api.newBundle(
        config.isPatch
          ? _.filter(file_list.files, function(f) {
              return f.match('.js$');
            })
          : null
      );
  ...
  config.buildPaths(env, function() {
    ...
    if (config.isAlloy && !config.skipAlloyCompile) {
      if (config.platform === undefined) {
        logger.error(
          'You need to use the --platform (android|ios) flag or have deployment-targets in tiapp.xml with an alloy project.'
        );
        process.exit();
      }
      async.detectSeries(
        ...
        function(platform, callback) {
          ...
          if (config.alloyCompileFile) {
            args[7] =
              'file=' + config.alloyCompileFile.split(',').join(',file=');
          }
          var alloy_command;
          if (config.useAppcCLI) {
            alloy_command = spawn('appc', ['alloy'].concat(args), {
              stdio: 'inherit'
            });
          } else {
...
function beginCompile(callback) {
  ...
  if (config.isUpdate) {
    ...
    if (
      file_list.files.length === 0 &&
      (!config.isModule || assets_list.files.length === 0) &&
      i18n_list.files.length === 0 &&
      spec_list.files.length === 0
    ) {
    ...
    fs.mkdirs([
      config.tishadow_build,
      config.tishadow_src,
      config.tishadow_dist
    ]);
  ...
  if (spec_list.files.length > 0) {
    ...
    spec_list.files = spec_list.files.map(function(file) {
      return 'spec/' + file;
    });
    spec_list.dirs = ['spec'].concat(
      spec_list.dirs.map(function(dir) {
        return 'spec/' + dir;
      })
    );
  }
  ...
  var process_tasks = file_list.files
    .map(function(file) {
      return _.bind(
        prepare,
        null,
        path.join(config.resources_path, file),
        path.join(config.tishadow_src, file)
      );
    })
    .concat(
      spec_list.files.map(function(file) {
        return _.bind(
          prepare,
          null,
          path.join(config.base, file),
          path.join(config.tishadow_src, file)
        );
      })
    );
  ...
  if (config.isModule && assets_list.files.length > 0) {
    ...
    process_tasks = process_tasks.concat(
      assets_list.files.map(function(file) {
        var fileName = file.slice(0, file.lastIndexOf('.'));
        if (fileName == config.module_name) {
          return _.bind(
            prepare,
            null,
            path.join(config.assets_path, file),
            path.join(config.tishadow_src, file)
          );
        } else {
          return _.bind(
            ...
            null,
            path.join(config.assets_path, file),
            path.join(config.tishadow_src, config.module_name, file)
          );
        }
      })
    );
  ...
  async.series([
    ...
    function() {
      file_list.files = file_list.files
        .concat(i18n_list.files)
        .concat(spec_list.files);
      if (config.isModule) {
