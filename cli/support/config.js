...
function getAppName(callback) {
  tiapp.find(process.cwd(), function(err, result) {
    if (err) {
      if (!config.globalCmd) {
        if (
          fs.existsSync(path.join(process.cwd(), 'timodule.xml')) &&
          fs.existsSync(path.join(process.cwd(), 'manifest'))
        ) {
...
config.buildPaths = function(env, callback) {
  ...
  getAppName(function(result) {
    if (!result) {
      // this is for the non-project specific commands
      callback();
      return;
    }
    ...
    config.bundle_file = path.join(
      config.tishadow_dist,
      config.bundle_name + '.zip'
    );
    config.jshint_path = fs.existsSync(config.alloy_path)
      ? config.alloy_path
    ...
    config.last_updated_file = path.join(
      config.tishadow_build,
      'last_updated' + (config.platform ? '_' + config.platform.join('_') : '')
    );
    config.isPatch = env.patch;
    config.isUpdate =
      (env.update || env.patch) &&
...
config.init = function(env) {
  ...
  config.globalCmd = _.contains(
    ['clear', 'close', 'screenshot', 'repl'],
    env._name
  );
  config.watchInterval = config.watchInterval || 100;
  config.watchDelay = config.watchDelay || 0;
  if (
    ['jasmine', 'mocha-chai', 'mocha-should', 'jasmine2'].indexOf(
      config.specType
    ) === -1
  ) {
    logger.error(
      'Invalid test library, please choose from: jasmine, mocha-should or mocha-chai'
    );
    process.exit(-1);
  }
  if (config.networkInterface) {
    var ifaces = os.networkInterfaces(),
      ipOfNtworkInterface = _.find(ifaces[config.networkInterface], function(
        ip
      ) {
  ...
  config.inspector =
    env.inspector !== undefined ? env.inspector : config.inspector || false;
  ...
  config.host =
    env.host || config.host || config.hostOfNetworkInterface || 'localhost';
  ...
  config.platform =
    env.platform && env.platform !== 'all'
      ? env.platform.split(',')
      : undefined;
  config.package_version = require('../../package.json').version;
  config.errorNotification =
    env.errorNotification !== undefined
      ? env.errorNotification
